...
// Constants for broadcast values used in semicolon matching
const BroadcastSemicolon = 0x3B3B3B3B3B3B3B3B
const Broadcast0x01 = 0x0101010101010101
const Broadcast0x80 = 0x8080808080808080

// stats3 holds statistical data for each station
type stats3 struct {
	min, max, count int32
	sum             int64
}

// processFile_3 processes the input file and returns the result as a string
func processFile_3(fileName string) string {
	...
	resultsCh := make(chan map[string]*stats3)
	for _, part := range parts {
		// Process each part concurrently
		go processPart_3(fileName, part.offset, part.size, resultsCh)
	}
...
// processPart_3 processes a part of the file and sends the result to the channel
func processPart_3(inputPath string, fileOffset, fileSize int64, resultsCh chan map[string]*stats3) {
